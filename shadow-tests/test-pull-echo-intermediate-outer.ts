...
/**
 * M9: when a replayed source commit has an echo'd source parent (i.e. one of
 * its parents on the source side carries a Shadow-replayed-<our-remote>
 * trailer pointing back to a commit on our side), the engine splices the
 * echo'd commit's outer tree into the replayed commit's parentTree.
 *
 * Without this splice, intermediate shadow commits inherit a stale outer
 * tree from the chain (typically frozen at bootstrap). With it, intermediate
...
export default function run() {
  ...
  try {
    ...
    // Push A/main to origin so composeShadowTip's splice (M1) sees x's outer.
    ...
    // The crucial M9 check: M_B''s tree should have mono.txt with x's
    // modification, NOT the bootstrap content. Without M9, mono.txt on
    // M_B' would be the original "internal repo root\n".
    const monoOnMerge = git(`show ${mergeReplayed}:mono.txt`, env.localRepo);
    assertEqual(
      monoOnMerge.trim(), "monorepo updated by x",
      "[M9] merge-replay's mono.txt should have x's modification (proves M9 spliced x's outer)",
    );

    // Tip should also match (this would pass even without M9, since M1
    // always applies to the tip via composeShadowTip).
    const monoOnTip = git(`show ${tipSha}:mono.txt`, env.localRepo);
    assertEqual(
      monoOnTip.trim(), "monorepo updated by x",
      "[M1] tip's mono.txt should have x's modification (M1 splice from target/main)",
    );

    // Sanity: the merge replay should have feature.ts under frontend/ from
    // x's contribution. (If M9's splice were wrong and clobbered shadow's
    ...
    assertEqual(
      ...
      "[M9] merge-replay's frontend/feature.ts should still have x's contribution",
    );
    ...
    // replayed onto shadow with frontend/ prefix). M9's splice keeps shadow's
    ...
    assertEqual(
      readmePresent, true,
      "[M9] merge-replay should still have frontend/README.md from earlier B-side commit",
    );
  } finally {
    env.cleanup();
  }
