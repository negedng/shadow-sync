/**
 * Regression test for the §5 variant in full_history_explained.html:
 ...
 * the §5 counterexample to "drop iff TREESAME-to-1st-parent." Under the
...
async function main() {
  ...
  try {
    ...
    banner("Final --from a (expected to succeed under the §5 discriminator)");
    r = await runSync({ from: "a" });
